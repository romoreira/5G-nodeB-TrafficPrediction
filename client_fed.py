...
params = {'batch_size': args.batch_size, 'epochs': 30, 'fc_dropout': 0.1, 'lr': 0.01, 'layers': [25], 'optimizer': Adam,
          'patience': 10}
...
params = {'batch_size': 32, 'epochs': 30, 'fc_dropout': 0.1, 'lr': 0.01, 'layers': [25], 'optimizer': Adam, 'patience': 10}
...
def check_error(orig, pred, name_col='', index_name=''):
    ...
    text_file = open("Resultados/"+str(model_name)+str("/")+str(str(model_name))+"_CLIENT-"+str(args.client_id)+"_error_measurement.txt", "w")
    n = text_file.write(str(result))
    text_file.close()
...
def plot_error(data, figsize=(12, 9), lags=24, rotation=0):
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig("Resultados/"+str(model_name)+"/" + str(model_name) + str("_CLIENT_")+str(args.client_id)+'_autoCorrelation.png', bbox_inches='tight', pad_inches=0.1)
